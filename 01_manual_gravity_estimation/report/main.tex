...
Estimar experimentalmente la aceleración de la gravedad ($g$) a partir de mediciones manuales de caída libre, empleando un ajuste por mínimos cuadrados no lineales sobre el modelo cinemático simplificado $y(t) = \frac{g}{2}t^2$.

\subsubsection{Objetivos Específicos}
\begin{itemize}
    \item Recopilar datos experimentales de tiempo y distancia de caída libre utilizando cronómetro manual.
    \item Implementar un script en Python que realice el ajuste numérico de los datos mediante la función \texttt{curve\_fit} de \texttt{SciPy}.
    \item Evaluar la calidad del ajuste a través de los residuos y la matriz de covarianza resultante.
    \item Identificar y analizar las fuentes de error sistemático asociadas a la medición manual del tiempo.
    \item Contrastar el valor obtenido de $g$ con el valor teórico esperado ($\approx 9.81\ \mathrm{m/s^2}$) y discutir las causas de la discrepancia.
\end{itemize}
...
Luego de correr el ajuste numérico en Python, se obtuvo un valor óptimo para la constante gravitacional de $g \approx 14.37\ \mathrm{m/s^2}$. Este valor excede de manera desproporcionada el valor esperado en condiciones reales. 
