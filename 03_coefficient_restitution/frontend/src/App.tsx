    ...
    <div className="min-h-screen bg-background text-white p-6 font-sans">
      <header className="mb-8 flex flex-col md:flex-row items-start md:items-center gap-4 border-b border-white/10 pb-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold">Coeficiente de Restitución</h1>
        </div>
        <div className="md:ml-auto flex gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0">
          {['Subir Video', 'Filtro HSV', 'Procesar', 'Resultados'].map((s, i) => (
            <div key={s} onClick={() => i + 1 <= step && setStep(i + 1)} className={`shrink-0 cursor-pointer px-4 py-2 text-sm rounded-full border transition-colors ${step === i + 1 ? 'bg-blue-600 border-blue-600 font-semibold' : 'border-zinc-700 text-zinc-400 hover:border-zinc-500'}`}>
              {i + 1}. {s}
            </div>
          ))}
        </div>
      ...
      <main className="max-w-6xl mx-auto">
        {step === 1 && (
          <div className="bg-card border border-white/5 p-12 rounded-2xl flex flex-col items-center justify-center min-h-[500px] text-center shadow-xl">
            <div className="w-20 h-20 bg-blue-500/10 rounded-full flex items-center justify-center mb-6">
              <Upload className="w-10 h-10 text-blue-500" />
            </div>
            <h2 className="text-2xl font-semibold mb-3">Paso 1: Seleccionar Video</h2>
            <p className="text-zinc-400 mb-8 max-w-sm">Elige un video de una pelota rebotando.</p>
            <label className="bg-blue-600 hover:bg-blue-500 transition-all text-white px-8 py-4 rounded-xl cursor-pointer font-bold text-lg shadow-lg">
            ...
            {videoInfo && (
              <div className="mt-8 w-full max-w-2xl flex flex-col items-center gap-6 animate-fade-in">
                ...
                <video src={`http://localhost:8002/video/${videoInfo.filename}`} controls className="max-h-[50vh] w-full bg-black rounded-lg shadow-xl" />
                <button onClick={() => setStep(2)} className="bg-emerald-600 hover:bg-emerald-500 shadow-lg text-white px-8 py-3 rounded-xl font-bold transition-all w-full md:w-auto">
                  Ir al Paso 2: Filtro HSV
                </button>
              </div>
            )}
          </div>
            ...
            <div className="lg:col-span-5 bg-card p-6 md:p-8 rounded-2xl border border-white/5 shadow-xl h-fit">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-zinc-400 text-sm">Ajusta los sliders para detectar la pelota.</p>
                </div>
              </div>
              <div className="mb-8 p-4 bg-zinc-900/50 rounded-xl border border-zinc-800">
                ...
                <input type="range" min={0} max={videoInfo.total_frames - 1} value={hsvFrame} onChange={e => setHsvFrame(parseInt(e.target.value))} className="w-full accent-blue-500" />
              </div>
              ...
              <button onClick={() => setStep(3)} className="w-full mt-8 bg-blue-600 hover:bg-blue-500 shadow-lg text-white py-4 rounded-xl font-bold transition-all text-lg">Confirmar HSV</button>
            </div>
        ...
        {step === 3 && (
          <div className="bg-card p-10 rounded-2xl border border-white/5 max-w-2xl mx-auto text-center mt-12 shadow-xl relative overflow-hidden">
            ...
            <p className="text-zinc-400 mb-8">El motor detectará los rebotes automáticamente.</p>
            <div className="flex gap-6 mb-10 p-6 bg-zinc-900/50 rounded-xl justify-center">
              <div>
            ...
            <button onClick={handleAnalyze} disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 shadow-xl disabled:opacity-50 text-white font-bold py-5 rounded-xl text-lg transition-all">
              {loading ? "⏳ Ejecutando..." : "Iniciar Análisis"}
            </button>
            {results && !loading && (
              ...
              <div className="mt-8 bg-[#0a0a0a] border border-zinc-800 rounded-xl p-4 text-left font-mono text-sm max-h-60 overflow-y-auto">
                {liveLogs.map((log, i) => (
                  <div key={i} className="mb-1 text-zinc-400">
                    {log.detected ? `✅ Bola Detectada » Frame: ${log.frame} | y=${log.y}px` : `❌ NO Detectado » Frame: ${log.frame}`}
                  </div>
                ))}
                <div ref={logEndRef} />
              </div>
          ...
          <div className="space-y-6">
            <div className="bg-card border border-white/5 rounded-2xl overflow-hidden shadow-xl">
              <div className="bg-black/40 px-6 py-4 border-b border-white/5">
                <h3 className="font-bold text-lg">Coeficiente de Restitución (e)</h3>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-[1px] bg-white/5">
                <div className="bg-card p-6">
                  <h4 className="text-sm text-zinc-500 mb-1">🎯 Promedio (e)</h4>
                  <div className="text-3xl font-mono font-bold text-blue-500">{results.e_avg.toFixed(3)}</div>
                </div>
                <div className="bg-card p-6">
                  <h4 className="text-sm text-zinc-500 mb-1">Desviación (±)</h4>
                  <div className="text-lg font-mono text-zinc-300">{results.e_std.toFixed(3)}</div>
                </div>
                <div className="bg-card p-6">
                  <h4 className="text-sm text-zinc-500 mb-1">Rebotes detectados</h4>
                  <div className="text-lg font-mono text-zinc-300">{results.e_values.length}</div>
                </div>
                <div className="bg-card p-6">
            ...
            <div className="bg-card border border-white/5 rounded-2xl p-6 shadow-xl h-[400px]">
              <h3 className="font-semibold text-lg mb-4 text-center">Trayectoria Vertical (Pixeles)</h3>
              <ResponsiveContainer width="100%" height="85%">
                <ScatterChart margin={{ top: 10, right: 10, bottom: 20, left: 10 }}>
                  ...
                  <YAxis type="number" dataKey="y" stroke="#71717a" domain={['auto', 'auto']} reversed>
                    <Label value="Y (px) ↓" angle={-90} position="insideLeft" fill="#71717a" />
                  </YAxis>
                  <ZAxis type="number" range={[15, 15]} />
                  <Tooltip contentStyle={{ backgroundColor: '#18181b', borderColor: '#27272a' }} />
                  <Scatter name="Trayectoria" data={results.times.map((t: any, i: any) => ({ time: t, y: results.y_pixels[i] }))} fill="#3b82f6" />
                </ScatterChart>
              </ResponsiveContainer>
            </div>
          </div>
