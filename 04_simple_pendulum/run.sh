...
echo -e "${BLUE}=== Starting Simple Pendulum App ===${NC}"

# Start Backend
echo -e "${GREEN}[+] Starting Backend (FastAPI on port 8003)...${NC}"
cd backend
if [ ! -d ".venv" ]; then
...
# Start Frontend
echo -e "${GREEN}[+] Starting Frontend (Vite/React on port 5176)...${NC}"
cd frontend
