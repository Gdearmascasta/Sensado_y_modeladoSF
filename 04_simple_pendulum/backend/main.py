...
app = FastAPI(title="Simple Pendulum API Workflow")
...
def preview_hsv(filename: str, frame_idx: int, hmin: int, hmax: int, smin: int, smax: int, vmin: int, vmax: int):
    ...
    center, mask = engine.detect_mass(frame, lower, upper)
...
def analyze_stream(
    ...
    return StreamingResponse(
        engine.process_video_stream(file_path, length, lower, upper, start_frame, end_frame),
...
def stream_detections(filename: str, hmin: int, hmax: int, smin: int, smax: int, vmin: int, vmax: int):
    ...
    def generate():
        ...
        while True:
            ...
            if not ret:
                break
            center, _ = engine.detect_mass(frame, lower, upper)
            if center:
