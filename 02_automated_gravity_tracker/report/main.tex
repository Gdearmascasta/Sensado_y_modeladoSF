...
\begin{titlepage}
	\centering
	\includegraphics[width=0.55\textwidth]{../../resources/logos/Nuevo-logo-UTB-1.png}\par\vspace{0.5cm} 
...
Desarrollar un sistema automatizado de análisis cinemático basado en visión computacional que permita estimar con alta precisión la aceleración de la gravedad ($g$) a partir de videos de objetos en caída libre, eliminando la intervención manual en la recolección de datos.

\subsubsection{Objetivos Específicos}
\begin{itemize}
    \item Implementar un algoritmo de segmentación por color en el espacio HSV para detectar y aislar el objeto en caída dentro de cada fotograma del video.
    \item Diseñar un sistema de calibración espacial que permita convertir las mediciones de píxeles a unidades métricas reales mediante un objeto de referencia.
    \item Calcular el centroide del objeto detectado cuadro a cuadro utilizando momentos de imagen ($M_{ij}$) para construir la trayectoria posición-tiempo.
    \item Aplicar un filtro heurístico basado en derivadas numéricas para aislar automáticamente el tramo de caída libre pura, descartando ruido estático y rebotes.
    \item Ajustar los datos filtrados al modelo parabólico $y(t) = y_0 + v_0 t + \frac{1}{2}gt^2$ mediante regresión no lineal (Levenberg-Marquardt) con tres grados de libertad simultáneos.
    \item Construir una arquitectura cliente-servidor asíncrona (FastAPI + React) que permita el procesamiento en tiempo real y la visualización interactiva de resultados.
\end{itemize}
...
\bibitem{github_repo} 
De Armas Castaño, G. (2026). 
\textit{Sensado y Modelado de Sistemas Físicos - Repositorio Oficial}. GitHub. Recuperado de \url{https://github.com/Gdearmascasta/Sensado_y_modeladoSF}
